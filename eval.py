...
#         print(dt_instances)
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, '/images/train')
...
if __name__ == '__main__':
    ...
    if "ICDAR15" in args.data_txt_path_val:
        args.mot_path = os.path.join(args.mot_path, "test")
        seq_nums = os.listdir(args.mot_path)
        args.mot_path = os.path.join(args.mot_path, "frames")

        print(args.mot_path,seq_nums)
...
#             continue
        if 'GT' not in seq_num.split('_') and 'Video' in seq_num.split('_'):
